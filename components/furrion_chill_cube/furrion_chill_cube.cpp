...
// is what re-bounds it, an over-floored crossing self-corrects in minutes via the e<0 unwind
// (live post-handover — the handover releases any armed raise freeze, and the freeze-intact
// suppression skips the floor entirely over a preserved bias), and max() still under-serves the
// drained-bias case the floor exists for. Revisit kd AND the cap
...
// the room sits ABOVE the band (cool; below, heat) and is NOT closing on SP — sustained for
// ADAPT_STALL_DWELL_MS — the biased upshift is allowed: in that regime the bias is not "stale
// climb data", it is a live measurement of exactly the load holding the room off SP.
// ADAPT_STALL_FALL_CPM: "closing" means moving toward SP faster than this (cool: drift ≤ -0.02;
// heat mirror: drift ≥ +0.02). 0.02 C/min = 2.2°F/h — the 08-15 livelock crept at ~0.013 C/min
// (87 min to close), which must count as stalled; genuine descents (the 06-30 hot-day windup
// case ran ≥0.05) stay gated by the anti-overshoot damping. Drift quantization is 0.06°F over
// the 3-6 min baseline ≈ 0.006-0.011 C/min, so 0.02 is ~2 quanta — a deliberate "too slow to
// matter" line, not a noise floor.
// ADAPT_STALL_DWELL_MS: the stall must PERSIST before the escape opens (bug-check 2026-08-15:
// hunt-cycle swing tops park the 3-min windowed slope inside ±0.02 for ~2 min every cycle — a
// dwell-free escape re-admitted the wound bias at exactly the moment the 06-30 gate was built
// for, reproducing the ungated top-gear pathology under tight ladder geometry, and could add a
// 1→2 upshift to mild-night hunts at bias ≳0.6). 6 min = 2× the drift window: transients can't
// accumulate it; the live livelock (9+ min) clears it with time to spare. The stamp resets the
// moment the room starts genuinely closing or drops into the band. Everything at/below the band
// is untouched (the 2026-07-02 stale-bias top-gear grab lives there).
static const float ADAPT_STALL_FALL_CPM = 0.02f;
static const uint32_t ADAPT_STALL_DWELL_MS = 360000;  // 6 min sustained stall before the escape opens
...
float FurrionChillCube::adaptive_cool_eff_diff_(float real_diff, uint32_t now, uint32_t time_in_gear) {
  ...
  // — except during an approach hold or an armed raise freeze, both deliberate suspensions),
  ...
  // Raise-side bias freeze maintenance (Stephen 2026-08-14, see the member note). THREE release
  // points: (1) here at the CROSSING (room back in the band from below); (2) an approach
  // HANDOVER (in the hold-maintenance blocks — demand re-established, bias re-lives with a
  // full-bias eff patch); (3) here at the horizon. Post-handover the e<0 unwind is deliberately
  // LIVE while the compressor runs on the re-lived bias (bug-check round 3, settled): once the
  ...
  // Upshift decisions see the learned bias while warming OR while PERSISTENTLY stalled above the
  // band (room above SP+deadband, fresh drift, not closing on SP, sustained ADAPT_STALL_DWELL_MS
  // — equilibrium above the band is the gate's premise failure, incident 2026-08-15; the dwell
  // keeps hunt-cycle swing tops from re-opening the gate, bug-check 2026-08-15). Otherwise fall
  // back to the unbiased diff (+ the fast fan feedforward, which is anticipatory and must still
  // act) — on that fallback leg, fminf guarantees the gate can only ever SUPPRESS an upshift,
  // never enable one: a stale NEGATIVE bias (eff < real_diff) keeps eff, so the fallback never
  ...
  if (stall_cond) {
    ...
    stall_above_since_c_ = 0;
    stall_logged_c_ = false;
  }
  bool stalled_above = (stall_above_since_c_ != 0) &&
                       (now - stall_above_since_c_ >= ADAPT_STALL_DWELL_MS);
  if (stalled_above && !stall_logged_c_) {
    stall_logged_c_ = true;
    ESP_LOGI(TAG, "Rate gate (cool): stalled above band %lu min (diff %.2f, drift %.3f C/min) — biased upshift enabled",
             (unsigned long)((now - stall_above_since_c_) / 60000UL), real_diff, room_drift_cpm_);
  }
...
float FurrionChillCube::adaptive_heat_eff_diff_(float real_diff, uint32_t now, uint32_t time_in_gear) {
  ...
  bool stalled_below = (stall_below_since_h_ != 0) &&
                       (now - stall_below_since_h_ >= ADAPT_STALL_DWELL_MS);
  if (stalled_below && !stall_logged_h_) {
    stall_logged_h_ = true;
    ESP_LOGI(TAG, "Rate gate (heat): stalled below band %lu min (diff %.2f, drift %.3f C/min) — biased upshift enabled",
             (unsigned long)((now - stall_below_since_h_) / 60000UL), real_diff, room_drift_cpm_);
  }
...
void FurrionChillCube::force_off_for_mode_switch_(uint32_t now) {
  ...
  update_action_();
  // Both callers return immediately after this teardown, skipping the pass-end debug publish —
  // publish here so the freeze/bias clears above are HA-visible this pass (bug-check round 3).
...
bool FurrionChillCube::check_failsafe_(uint32_t now, float room) {
  ...
  if (never_got_update || ha_disconnected || temp_unavailable) {
    ...
    // freeze at their pre-failsafe values for the whole episode — debug_raise_freeze would show
    // a phantom armed freeze, and debug_failsafe_active could never publish 1 at all (set above,
    // cleared before the next publish-reaching pass).
    publish_debug_state_(NAN);
    return true;
  }
...
float FurrionChillCube::apply_crossing_preload_(bool is_heat, uint32_t now, bool freeze_intact) {
  ...
  // Age bound (bug-check round 1; lead corrected round 2): an idle-fired hold can legally run to
  // its 3×-lead hard cap (~45 min at the 15-min lead) before the band release — a snapshot that
  // old is a stale load estimate. Bound on the LARGER of the two leads (an OFF-fired hold engages
  // on approach_lead_off, which a config may set above the idle lead; the ~305 s clamp ride is
  // comfortably inside 2× either lead in any sane config). 64-bit math so an absurd configured
  // lead cannot wrap the doubling.
  uint32_t bound_lead = (approach_lead_off_ms_ > approach_lead_ms_) ? approach_lead_off_ms_
                                                                    : approach_lead_ms_;
  if ((uint64_t)(now - snap_at) > 2ULL * (uint64_t) bound_lead) return 0.0f;
  ...
  // Freeze-intact suppression (bug-check round 2; log placed AFTER the viability gates round 3
  // so it never claims the freeze blocked a floor that was stale/wrong-direction anyway): the
  // floor's charter is the DRAINED-bias entry (long park, unwound morning). A handover that
  // found the raise freeze armed carries the bias the freeze preserved — a direct measurement
  // of this load — and the cap-clipped estimator must not overwrite it via max(). Consume the
  // snapshot, skip the floor. (Residual accepted: a clamp landing 1+ pass AFTER the crossing
  // release sees freeze_intact=false and floors the still-intact bias — worst case
  // kd × cap − preserved ≈ 1.7 over a barely-armed 0.1 bias, ~0.3 in the 1.5 payoff shape;
  // self-corrects via the post-release e<0/e>0 integration.)
  if (freeze_intact) {
    ESP_LOGI(TAG, "Crossing preload (%s): floor %.2f skipped — freeze-preserved bias %.2f is the trusted measurement",
             is_heat ? "heat" : "cool", floor_b, bias);
    return 0.0f;
  }
    ...
    if (isnan(last_committed_heat_target_c_)) {
      ...
      if (delta_c > 0.0f && adaptive_enable_ && sp_preload_factor_ > 0.0f &&
        ...
        ESP_LOGI(TAG, "Heat SP raise %.2fC: bias preloaded +%.2f -> %.2fC", delta_c, preload, bias_h_);
                 ...
                 (bias_h_ > NATURAL_OFF_BIAS_EPS_C || raise_freeze_h_at_ != 0)) {
        ...
        if (raise_freeze_h_at_ != 0 && (now - raise_freeze_h_at_) >= RAISE_FREEZE_MAX_MS) {
          ...
          ESP_LOGI(TAG, "Heat SP drop %.2fC: freeze already armed %lu min ago — horizon unchanged",
                   -delta_c, (unsigned long)((now - raise_freeze_h_at_) / 60000UL));
        }
      }
    }
  }
...
bool FurrionChillCube::run_cool_mode_(float room, uint32_t now, bool user_input, bool from_test,
  ...
  // by the never-frozen e<0 path (a DEMAND-RESTORING drop mid-freeze — room above the new target
  // past the deadband — releases the freeze the same pass via the crossing test, whether or not
  // it preloads; a drop that leaves the room still below the new target keeps the freeze armed
    ...
    if (isnan(last_committed_cool_target_c_)) {
      ...
      if (delta_c < 0.0f && adaptive_enable_ && sp_preload_factor_ > 0.0f &&
        ...
        ESP_LOGI(TAG, "Cool SP drop %.2fC: bias preloaded +%.2f -> %.2fC", -delta_c, preload, bias_c_);
                 ...
                 (bias_c_ > NATURAL_OFF_BIAS_EPS_C || raise_freeze_c_at_ != 0)) {
        // Arm only over a bias worth preserving (bug-check round 2): the freeze's premise —
        // "the retained bias IS the load measurement" — holds only for a meaningful POSITIVE
        // bias. Near-zero has nothing to protect, and a NEGATIVE bias must stay live/decaying
        // (freezing it out would make eff HIGHER right after a make-it-warmer command). The
        // already-armed leg keeps re-raises inside an active freeze recognized (bias > EPS is
        // invariant while armed anyway — belt and suspenders).
        ...
        if (raise_freeze_c_at_ != 0 && (now - raise_freeze_c_at_) >= RAISE_FREEZE_MAX_MS) {
          ...
          ESP_LOGI(TAG, "Cool SP raise %.2fC: freeze already armed %lu min ago — horizon unchanged",
                   delta_c, (unsigned long)((now - raise_freeze_c_at_) / 60000UL));
        }
      }
    }
  }
...
void FurrionChillCube::publish_debug_state_(float diff) {
  ...
  pub(debug_raise_freeze_sensor_,
      (raise_freeze_c_at_ != 0) ? 1.0f : ((raise_freeze_h_at_ != 0) ? 2.0f : 0.0f));
